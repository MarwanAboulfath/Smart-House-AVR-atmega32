...
static u8 * SPI_pu8TData = NULL_ptr ;

/*Global variable to carry the Receive Data*/
static u8 * SPI_pu8RData = NULL_ptr ;
...
static void (* SPI_pvNotificationFunc)(void)= NULL_ptr;
...
u8 SPI_u8InitConfig (SPI_CONFIG * spi)
{
	...
	if ((spi != NULL_ptr) && (spi->SPI_u8ClockPhase != 0) && (spi->SPI_u8ClockPolarity != 0) && (spi->SPI_u8ClockRate != 0) && (spi->SPI_u8DataOrder != 0) && (spi->SPI_u8InterruptEnable != 0) && (spi->SPI_u8MasterSlaveSelect != 0) && (spi->SPI_u8SPIEnable != 0))
	{
...
u8 SPI_u8Tranceive (u8 Copy_u8TData , u8 * Copy_u8RData)
{
	...
	if (SPI_u8State == IDLE)
	{
		...
		{
			Local_u8ErrorState = E_NOK_TIME_OUT ;
		}
		else
		{
			* Copy_u8RData = SPDR ;
...
u8 SPI_u8BufferTranceiverSynch (u8 * Copy_u8TData , u8 * Copy_u8RData , u8 Copy_u8BufferSize)
{
	...
	if ((Copy_u8TData != NULL_ptr) && (Copy_u8RData != NULL_ptr))
	{
...
u8 SPI_u8BufferTranceiverAsynch (SPI_BUFFER * spi_buffer)
{
	...
	if (SPI_u8State == IDLE)
	{
		if ((spi_buffer != NULL_ptr) && (spi_buffer->Copy_u8TData != NULL_ptr) && (spi_buffer->Copy_u8RData != NULL_ptr) && (spi_buffer->NotificationFuncn != NULL_ptr))
		{
			/*SPI is now Busy*/
			SPI_u8State = BUSY ;
...
void SPI_voidSlaveSetData(u8 Copy_u8TData)
{
	SPDR = Copy_u8TData;
}
