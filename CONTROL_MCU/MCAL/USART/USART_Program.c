...
static const u8 * USART_pu8SendData = NULL_ptr ;

/*Global variable to carry the Receive Data*/
static u8 * USART_pu8ReceiveData = NULL_ptr ;
...
static void (* USART_pvNotificationFunc)(void)= NULL_ptr;
...
u8 USART_u8RecevieData (u8 * Copy_u8ReceviedData)
{
	...
	u32 Local_u32TimeoutCounter = 0 ;
	if (Copy_u8ReceviedData != NULL_ptr)
	{
		if (USART_u8State == IDLE)
		{
			USART_u8State = BUSY ;
...
u8   USART_u8SendStringAsynch (const u8 * Copy_pchString , void (* NotificationFunc)(void))
{
	...
	if (USART_u8State == IDLE)
	{
		if ((Copy_pchString != NULL_ptr) && (NotificationFunc != NULL_ptr))
		{
			/*USART is now Busy*/
			USART_u8State = BUSY ;
